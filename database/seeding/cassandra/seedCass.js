...
const dataGen = async () => {
  const createRestaurantReservationsTable = async () => {
    writer.pipe(fs.createWriteStream('restaurantReservations.csv'));
    for (let i = 0; i < 250; i++) {
      writer.write({
        restaraunt_id: faker.random.number({min:1, max: 10000000}),
...
//     writer3.pipe(fs.createWriteStream('reservationInfo13.csv'));
